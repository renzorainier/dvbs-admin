...
const StudentRanking = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db2, "points/config"), (querySnapshot) => {
      ...
      if (!groupToShow) {
        console.log("No group to show configured.");
        return;
      }

      const unsubscribeStudents = onSnapshot(collection(db, "dvbs"), (querySnapshot) => {
        ...
        // Filter students by the configured group
        const studentsToShow = presentStudents.filter((student) => student.group === groupToShow);

        // Sort students by points from highest to lowest
        studentsToShow.sort((a, b) => b.points - a.points);

        // Group students by their ranks within the group
        const groupedByRank = {};
        let rank = 1;
        studentsToShow.forEach((student) => {
          ...
          if (groupedByRank[rank].length < 5) {
            rank++;
          }
        });
