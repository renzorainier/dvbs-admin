...
function Schedule() {
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      if (currentConfig.name === "All") {
        const allData = {};
        for (const config of configurations.slice(0, -1)) {
          const docRef = doc(db3, config.db3Path.split("/")[0], config.db3Path.split("/")[1]);
          const scheduleSnapshot = await getDoc(docRef);
          if (scheduleSnapshot.exists()) {
            const data = scheduleSnapshot.data();
            allData[config.name] = data;
          } else {
            console.error(`No such document for ${config.name}!`);
          }
        }
        setScheduleData(allData);
      } else {
        const docRef = doc(db3, currentConfig.db3Path.split("/")[0], currentConfig.db3Path.split("/")[1]);
        const scheduleSnapshot = await getDoc(docRef);
        if (scheduleSnapshot.exists()) {
          const data = scheduleSnapshot.data();
  ...
  }, [currentConfig.db3Path, currentConfig.name]);
  ...
  const renderSchedule = () => {
    if (currentConfig.name === "All") {
      return Object.keys(scheduleData).map((group) => (
        <div key={group}>
          <h2 className="text-2xl font-bold text-center my-4">{group}</h2>
          {renderGroupSchedule(scheduleData[group], configurations.find(config => config.name === group).color)}
        </div>
      ));
    } else {
      return renderGroupSchedule(scheduleData, currentConfig.color);
    }
  };

  const renderGroupSchedule = (data, color) => {
    ...
    const filteredSegments = segments.filter((segment) => {
      const endTime = data[`${segment}end`];
    ...
    return [...(lastPastSegment ? [lastPastSegment] : []), ...filteredSegments].map((segment, index, array) => {
      ...
      return (
        ...
        >
          <h3 className={`${isCurrent ? "text-2xl text-center text-white" : "text-lg"} font-bold`}>{data[segment]}</h3>
          <p className={`${isCurrent ? "text-lg text-center" : "text-sm"}`}>
            <strong>Location:</strong> {data[`${segment}loc`]}
          </p>
          <p className={`${isCurrent ? "text-lg text-center" : "text-sm"}`}>
