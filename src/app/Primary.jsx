...
import { doc, updateDoc, getDoc, batch } from "firebase/firestore";
...
function Primary({ config, currentConfigIndex, setCurrentConfigIndex }) {
  ...
  const [updatesBatch, setUpdatesBatch] = useState([]);
  ...
  const handleClick = (fieldName) => {
    ...
    if (primaryData[fieldToUpdate]) {
      ...
      addUpdateToBatch(fieldName, fieldToUpdate);
    }
  };

  const addUpdateToBatch = (fieldName, fieldToUpdate) => {
    ...
    setUpdatesBatch((prevBatch) => [...prevBatch, { fieldName, updateData }]);
  };
  ...
  const executeBatchUpdate = async () => {
    try {
      ...
      updatesBatch.forEach(({ fieldName, updateData }) => {
        const docRef = doc(
          db,
          config.dbPath.split("/")[0],
          config.dbPath.split("/")[1]
        );
        batchUpdate.update(docRef, updateData);
      });

      await batchUpdate.commit();

      // Update local state after successful batch update
      const updatedPrimaryData = { ...primaryData };
      updatesBatch.forEach(({ fieldName, updateData }) => {
        Object.keys(updateData).forEach((key) => {
          updatedPrimaryData[key] = updateData[key];
        });
      });
